...
/* ---------- tableau ADMIN prestations (label, key, PV TTC, CR m²) ---------- */
function renderAdminServices() {
  ...
  cfg.services.forEach((row, idx) => {
    ...
    inPV.addEventListener('input', () => {
      ...
      renderAllNumbers(); // répercute PV sur devis
    });
  ...
  if (btnAdd && !btnAdd.__bound) {
    ...
    btnAdd.addEventListener('click', () => {
      ...
      updateConfig(c => {
        if (!c.services.some(s => s.key === key)) {
...
function renderCostsTable() {
  ...
  (cfg.services || []).forEach((s) => {
    ...
    input.value = Number.isFinite(cfg.costsM2?.[s.key]) ? String(cfg.costsM2[s.key]) : '';
...
function renderServicesAndRecap() {
  ...
  ORDER().forEach((key) => {
    ...
    const costMeuble = effM2 * surface;
    ...
    left.innerHTML = `<span class="inline-block h-2.5 w-2.5 rounded-full ${selected ? 'bg-emerald-500' : 'bg-neutral-300'}"></span>
                      <span>${LABEL(key)}</span>`;
    ...
    right.innerHTML = `
      ...
      <div>${Number.isFinite(costM2) ? euro(costMeuble) + ' /meuble' : '— € /meuble'}</div>
    `;
...
export function initCR() {
  // Applique la config courante à PRICING
  ...
  // sync live depuis Devis et depuis Admin
